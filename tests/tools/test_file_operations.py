...
class TestShellFileOpsAtomicWrite:
    def test_atomic_write_wraps_errexit_in_subshell(self, mock_env):
        """Atomic writes use `set -e`, but it must not leak into Hermes'
        wrapper shell. The wrapper refreshes its environment snapshot after
        each command, and bash `declare -f | grep ...` can legitimately return
        1 when no functions exist; leaked errexit turns a successful write into
        a false failure.
        """
        ops = ShellFileOperations(mock_env)
