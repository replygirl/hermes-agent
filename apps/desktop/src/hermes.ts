...
export const hermesDesktopConsole = {
  ...
  getSession: (id: string, profile?: string | null) => {
    const suffix = profile ? `?profile=${encodeURIComponent(profile)}` : ''
    return consoleRawApi<SessionInfo>({ path: `/api/sessions/${encodeURIComponent(id)}${suffix}` })
  },
  getSessionMessages: (id: string, profile?: string | null) => {
    const suffix = profile ? `?profile=${encodeURIComponent(profile)}` : ''
    return consoleRawApi<SessionMessagesResponse>({ path: `/api/sessions/${encodeURIComponent(id)}/messages${suffix}` })
  },
